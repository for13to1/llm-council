...
from .config import CHAIRMAN_MODEL, COUNCIL_MODELS, TITLE_MODEL, model_display_name
...
async def stage2_collect_rankings(
    ...
    responses_text = "\n\n".join(
        [f"Response {label}:\n{result['response']}" for label, result in zip(labels, stage1_results, strict=True)]
    )
...
Provide a clear, well-reasoned final answer that represents the council's collective wisdom:"""
    ...
    chairman_name = model_display_name(CHAIRMAN_MODEL)
...
Title:"""
    ...
    # Use configured title model (fast and cheap)
